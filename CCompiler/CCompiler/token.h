...
public:
    ...
    token(string value, string type, tokenType tokentype):
            value(value), type(type), tokentype(tokentype) {}
    ...
    void printString(string i) {
        printf("%s%s\n",i.c_str(),this->value.c_str());
        if (children.size() != 0) {
//            printf("%s%s",i.c_str(),this->value.c_str());
...
//    newToken.value = actualValue;
    if (reserve.count(actualValue) != 0) {
        ...
        newToken = token(actualValue, "Reserve", reserve[actualValue]);
//        newToken.tokentype = reserve[actualValue];
//        newToken.type = "Reserve";
    } else {
//        newToken.tokentype = tokentype;
//        newToken.type = type;
        newToken = token(actualValue, type, tokentype);
    }
...
//        plusNewToken.type = "Symbol";
        if (actualValue[0] == '+') {
            plusNewToken = token("+", "Symbol", PLUS);
//            plusNewToken.value = '+';
//            plusNewToken.tokentype = PLUS;
        } else {
            plusNewToken = token("-", "Symbol", MINUS);
//            plusNewToken.value = '-';
