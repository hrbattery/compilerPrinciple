...
//                    printf("Action stat: %d, action from %d ACCEPT %d with symbol %s.\n", ACCEPT, i, it.prod.num, token_strs[t]);
//                } else if (it.lookAhead.count(t) != 0 || (C[i].items.size() == 1 && reducableItems.size() == 1) ){
                } else if (it.lookAhead.count(t) != 0 || reducableItems.size() == 1 ){
                    pair<actionStat, int> p = make_pair(REDUCE, it.prod.num);
                    if (ACTION.count(make_pair(i, t)) == 0 && s.gotoSymbols.find(t) == s.gotoSymbols.end()) {
                        ACTION[make_pair(i, t)] = p;
//                        printf("Action stat: %d, action from %d reducing %d with symbol %s.\n", REDUCE, i, it.prod.num, token_strs[t]);
                    } else if (ACTION.count(make_pair(i, t)) != 0) {
//                        printf("Error: conflict. action stat: %d, action from %d reducing %d with symbol %s.\n", REDUCE, i, it.prod.num, token_strs[t]);
                    }
                }
            }
...
//                        if (it.lookAhead.count(t) != 0){
                        ...
                        if (ACTION.count(make_pair(i, t)) == 0) {
                            ACTION[make_pair(i, t)] = p;
//                            printf("Action stat: %d, action from %d to %d with symbol %s.\n", SHIFT, i, stateNum, token_strs[t]);
                        } else {
...
//                                    printf("Error: conflict. action stat: %d, action from %d to %d with symbol %s.\n", SHIFT, i, stateNum, token_strs[t]);
                                }
...
token LALR::parser() {
    ...
    token head = token("start", "nonTerminal", start);
    while (!success) {
//        printf("%s{", step == 0?"\n":"\n");
//        printf("\"statusStack\": \"");
//        showStatusStack();
...
//            printf("\"REDUCE %s->%s\"}", pr.leftStr().c_str(), pr.rightStr().c_str());
            token newToken = token(token_strs[pr.left], "nonTerminal", pr.left);
            if (pr.right.size() == 0) {}// 空串，无需出栈，直接规约
            for (unsigned i=0; i<pr.right.size(); ++i) {
                status.pop_back();
                newToken.addChild(parse[parse.size()-1]);
                parse.pop_back();
            }
